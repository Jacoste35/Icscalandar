...
// Activé dès qu'une clé est fournie via l'environnement (DATA_ENCRYPTION_KEY).
// La clé (phrase secrète) est dérivée en 32 octets par SHA-256. Le fichier (ou
// la valeur Redis) est alors illisible sans la clé. Rétro-compatible : un
// contenu en clair est lu normalement et re-chiffré au prochain enregistrement.
const ENC_PASS = process.env.DATA_ENCRYPTION_KEY || '';
const ENC_KEY = ENC_PASS ? crypto.createHash('sha256').update(String(ENC_PASS)).digest() : null;
const ENC_PREFIX = 'ICSENC1:';
...
function isEncrypted(s) { return typeof s === 'string' && s.startsWith(ENC_PREFIX); }

// --- Chiffrement AU NIVEAU DES CHAMPS pour les données personnelles (RGPD) ---
// Les coordonnées personnelles des salariés (e-mail, téléphone, adresse, date de
// naissance) ne sont JAMAIS stockées en clair : elles sont chiffrées (AES-256-GCM)
// au moment de l'enregistrement et déchiffrées au chargement. En mémoire vive,
// l'application manipule des valeurs en clair (le code existant n'est pas modifié).
// La clé est dérivée d'un secret stable (DATA_ENCRYPTION_KEY de préférence).
const PII_PASS = process.env.PII_ENCRYPTION_KEY || process.env.DATA_ENCRYPTION_KEY || process.env.JWT_SECRET || 'ics-pii-fallback-key-change-me';
const PII_KEY = crypto.createHash('sha256').update(String(PII_PASS)).digest();
const PII_PREFIX = 'PII1:';
const PII_FIELDS = ['email', 'phone', 'address', 'birthDate'];
function piiEnc(plain) {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', PII_KEY, iv);
  const ct = Buffer.concat([cipher.update(String(plain), 'utf8'), cipher.final()]);
  return PII_PREFIX + Buffer.concat([iv, cipher.getAuthTag(), ct]).toString('base64');
}
function piiDec(blob) {
  try {
    const raw = Buffer.from(String(blob).slice(PII_PREFIX.length), 'base64');
    const iv = raw.subarray(0, 12), tag = raw.subarray(12, 28), ct = raw.subarray(28);
    const d = crypto.createDecipheriv('aes-256-gcm', PII_KEY, iv);
    d.setAuthTag(tag);
    return Buffer.concat([d.update(ct), d.final()]).toString('utf8');
  } catch (e) { return blob; } // jamais de perte : on renvoie la valeur telle quelle
}
// Chiffre les champs PII des utilisateurs EN PLACE (synchrone), renvoie une
// fonction qui restaure les valeurs en clair. Aucune opération asynchrone ne
// s'intercale entre le chiffrement et la restauration → pas de fuite concurrente.
function encryptPiiInPlace(obj) {
  const saved = [];
  for (const u of (obj && obj.users) || []) {
    for (const f of PII_FIELDS) {
      const v = u[f];
      if (typeof v === 'string' && v && !v.startsWith(PII_PREFIX)) { saved.push([u, f, v]); u[f] = piiEnc(v); }
    }
  }
  return () => { for (const [u, f, v] of saved) u[f] = v; };
}
function decryptPiiInData(obj) {
  for (const u of (obj && obj.users) || []) {
    for (const f of PII_FIELDS) {
      const v = u[f];
      if (typeof v === 'string' && v.startsWith(PII_PREFIX)) u[f] = piiDec(v);
    }
  }
  return obj;
}

// Sérialise/désérialise en tenant compte du chiffrement éventuel + PII chiffrées.
function serialize(obj) {
  const restore = encryptPiiInPlace(obj);
  let json;
  try { json = JSON.stringify(obj, null, ENC_KEY ? 0 : 2); } finally { restore(); }
  return ENC_KEY ? encryptData(json) : json;
}
function deserialize(blob) {
  if (isEncrypted(blob)) {
    if (!ENC_KEY) throw new Error('Données chiffrées : clé DATA_ENCRYPTION_KEY absente. Refus de réinitialiser la base.');
    return decryptPiiInData(JSON.parse(decryptData(blob)));
  }
  return decryptPiiInData(JSON.parse(blob));
}
...
const DEFAULT_DATA = {
  ...
  vehicleSchedule: [],     // entretiens à programmer (libres) : { id, vehicleId, label, dueKm, dueDate, note, done }
  vehicleKmLog: [],        // km parcourus relevés via les rapports : { id, vehicleId, date, km, userId, userName, source }
  kmAnomalies: [],         // anomalies de km détectées à l'import (validation admin) : { id, vehicleId, vehicleName, plate, date, km, userId, userName, reason, createdAt }
  // Messagerie interne (annonces de l'encadrement aux salariés, accusés de lecture).
  messages: [],            // { id, authorId, authorName, title, body, createdAt, reads:[{userId,name,at}] }
  // Candidatures spontanées reçues depuis la page de connexion (CV + lettre + message).
  candidatures: [],        // { id, firstName, lastName, email, phone, message, cv:{name,type,data}, letter:{...}, status:'new'|'read'|'archived', createdAt }
  // Bot WhatsApp : file des messages sortants + échanges salarié ↔ direction.
  waOutbox: [],            // { id, phone|group, userId, text, createdAt, sentAt }
  botMessages: [],         // { id, userId, userName, from:'employee'|'direction', text, at, handledBy }
  // Retours de tournée (saisis via WhatsApp après retour au dépôt).
  tourReturns: [],         // { id, date, vehicleId, userId, groupId, depotKey, tourNo, parcels, points, revenue, status, createdAt }
  // Clients professionnels (destinations récurrentes) pour l'optimisateur de tournée.
  proClients: [],          // { id, name, address, lat, lon, horaires, horairesSource, horairesMajLe, note, createdAt }
  // Stock de pièces / consommables (avec prix) pour le coût d'exploitation.
  parts: [],               // { id, name, ref, category, unitPrice, qty, unit }
  // Dépenses par véhicule (entretien, carburant…) pour le coût réel d'exploitation.
  vehicleExpenses: [],     // { id, vehicleId, date, category, label, amount, partId, qty, km }
  // Comptabilité simplifiée (recettes / charges, TVA, par client).
  finance: { entries: [] },// entry: { id, ym, kind, category, client, amount, fixed, vatRate, note }
  // Contrats donneurs d'ordre (GLS, FedEx, DPD, Chronopost, UPS…).
  contracts: [],
  // Avertissements / sanctions archivés (suite de rappels disciplinaires).
  sanctions: [],           // { id, userId, userName, type, date, motif, createdBy, createdByName, createdAt }
  // Heures de travail / amplitudes des chauffeurs (Gestion des heures).
  workHours: [],           // { id, userId, userName, date, start, end, breakMin, amplitude, worked }
  // Règlement des heures supplémentaires par salarié et par mois.
  hsupSettlements: [],     // { userId, month, paidHours, transmittedEquiv }
  // Éléments de paie importés des bulletins (heures sup. 25 %, majoration nuit,
  // indemnité de repas) par salarié et par mois.
  payImports: [],          // { id, userId, userName, month, hsup25, nightHours, mealCount, mealAmount, source, createdAt }
  // Écritures bancaires importées (relevés) — module Gestion Financière.
  bankTx: [],              // { id, opDate, valDate, label, debit, credit, amount, category, subCategory, bank, account, source, hash }
  // Documents importés (métadonnées des relevés archivés).
  bankDocs: [],            // { id, name, bank, importedAt, lines }
  ...
  settings: {
    ...
    // Alertes de conformité véhicule marquées « lues » (masquées de l'accueil).
    vehicleWarnAcks: {},
    // Rappels disciplinaires clôturés (masqués de l'accueil) : { userId: true }.
    disciplineAcks: {},
    // Règles de catégorisation bancaire (mot-clé -> catégorie) + apprentissage.
    catRules: [
      { kw: 'TOTAL', cat: 'Carburant' }, { kw: 'ESSO', cat: 'Carburant' }, { kw: 'SHELL', cat: 'Carburant' }, { kw: 'AVIA', cat: 'Carburant' }, { kw: 'STATION', cat: 'Carburant' },
      { kw: 'VINCI', cat: 'Péages' }, { kw: 'APRR', cat: 'Péages' }, { kw: 'BIP&GO', cat: 'Péages' }, { kw: 'SANEF', cat: 'Péages' },
      { kw: 'SALAIRE', cat: 'Salaires' }, { kw: 'PAYFIT', cat: 'Salaires' }, { kw: 'SILAE', cat: 'Salaires' },
      { kw: 'URSSAF', cat: 'Charges sociales' }, { kw: 'MSA', cat: 'Charges sociales' }, { kw: 'POLE EMPLOI', cat: 'Charges sociales' },
      { kw: 'AXA', cat: 'Assurances' }, { kw: 'ALLIANZ', cat: 'Assurances' }, { kw: 'GROUPAMA', cat: 'Assurances' }, { kw: 'ASSURANCE', cat: 'Assurances' },
      { kw: 'ORANGE', cat: 'Téléphonie' }, { kw: 'SFR', cat: 'Téléphonie' }, { kw: 'BOUYGUES', cat: 'Téléphonie' }, { kw: 'FREE', cat: 'Téléphonie' },
      { kw: 'PNEU', cat: 'Pneumatiques' }, { kw: 'MASSA', cat: 'Pneumatiques' },
      { kw: 'LIXXBAIL', cat: 'Véhicules (leasing)' }, { kw: 'COFICA', cat: 'Véhicules (leasing)' }, { kw: 'LEASE', cat: 'Véhicules (leasing)' },
      { kw: 'LOYER', cat: 'Loyer' }, { kw: 'CUVIGNY', cat: 'Loyer' },
      { kw: 'GLS', cat: 'Chiffre d\'affaires' }, { kw: 'GENERAL LOGISTICS', cat: 'Chiffre d\'affaires' }, { kw: 'FEDEX', cat: 'Chiffre d\'affaires' }, { kw: 'DPD', cat: 'Chiffre d\'affaires' }, { kw: 'CHRONOPOST', cat: 'Chiffre d\'affaires' }, { kw: 'UPS', cat: 'Chiffre d\'affaires' }, { kw: 'CIBLEX', cat: 'Chiffre d\'affaires' }, { kw: 'LA POSTE', cat: 'Chiffre d\'affaires' }, { kw: 'AS24', cat: 'Chiffre d\'affaires' }, { kw: 'RELAIS COLIS', cat: 'Chiffre d\'affaires' },
      { kw: 'COM/VIR', cat: 'Frais bancaires' }, { kw: 'COTISATION', cat: 'Frais bancaires' }, { kw: 'AGIOS', cat: 'Frais bancaires' },
    ],
    catLearn: {},            // libellé normalisé -> catégorie (apprentissage)
    // Apprentissage des motifs d'absence lus sur les bulletins : libellé
    // normalisé -> code de catégorie du site (confirmé par l'administrateur).
    motifLearning: {},
    treasuryStartBalance: 0, // solde de trésorerie de départ
    // Paramètres d'estimation d'appel d'offre (prix de livraison au point).
    // Effectif réel : 18 chauffeurs, 2 responsables, 1 secrétaire ; 8 h/jour, 40 h/sem.
    tenderParams: {
      nbDrivers: 18, nbResponsables: 2, nbSecretaires: 1,
      hoursPerDay: 8, hoursPerWeek: 40, daysPerMonth: 21,
      driverCost: 2900,       // coût chauffeur mensuel chargé (40 h/sem) €
      responsableCost: 3500,  // coût responsable mensuel chargé €
      secretaireCost: 2600,   // coût secrétaire mensuel chargé €
      fraisGeneraux: 6000,    // loyer, télécom, assurances structure… /mois €
      vehicleCost: 1300,      // leasing + assurance + entretien mensuel /véhicule €
      consumption: 9,         // consommation L/100 km
      fuelPrice: 1.75,        // prix du gasoil €/L
      tyresPerMonth: 120,     // pneumatiques amortis / mois / véhicule €
      pointsPerDay: 90,       // points livrés / jour / chauffeur
      kmPerDay: 120,          // km / jour / chauffeur
      marginPct: 10,          // marge / bénéfice cible (%)
      fuelSurchargePct: 10,   // part gasoil indexée (6 / 8 / 10 / 12 %)
      cnrRef: 100,            // indice gasoil CNR de référence (base 100)
      cnrCurrent: 100,        // indice gasoil CNR M-1 (base 100) — cnr.fr
      ramassage: 0,           // 1 si l'offre inclut aussi le ramassage
      ramassagePerDay: 0,     // enlèvements / jour / chauffeur
      ramassagePrice: 0,      // prix par enlèvement € (mode manuel)
      ramassageMode: 1,       // 0 = prix saisi · 1 = auto-équilibré sur la même base
      ramassageRatio: 0.7,    // mode auto : prix enlèvement = ratio × prix livraison
      marginPremiumPct: 18,   // marge « premium » (%)
      targetPtsPerHour: 15,   // seuil de productivité (points/heure) « bon »
      maxKmPerPoint: 2,       // seuil km/point au-delà duquel densité insuffisante
      // Pôles « méthode ICS » (fichier tarif au point) — valeurs de référence.
      panierRepas: 16.36,     // indemnité repas exonérée €/jour/chauffeur
      joursPanier: 21.67,     // jours de panier / mois
      remplacementPerDriver: 263, // provision remplacement congés €/mois/chauffeur
      entretienPerKm: 0.0353, // entretien & consommables €/km (pneus+plaq.+disq.+révision)
      auDelaPct: 50,          // tarif livraison au-delà du forfait = % du prix de base
    },
    // Catégories et unités de pièces (paramétrables) pour le stock.
    partCategories: ['Filtre à huile', 'Filtre habitacle', 'Filtre à air', 'Filtre à gasoil', 'Huile moteur 5W30', 'Freins', 'Pneus', 'Courroies', 'Embrayage', 'Éclairage', 'Batterie', 'Carrosserie', 'Carburant', 'Divers'],
    partUnits: ['u', 'jeu', 'paire', 'L', 'kg', 'm'],
  },
};
...
function migrate(d) {
  ...
  if (!Array.isArray(d.vehicleKmLog)) d.vehicleKmLog = [];
  if (!Array.isArray(d.kmAnomalies)) d.kmAnomalies = [];
  ...
  for (const v of d.vehicles) {
    ...
    if (v.firstRegistration === undefined) v.firstRegistration = null; // 1re mise en circulation
    if (!Array.isArray(v.ctControls)) v.ctControls = [];     // [{id,date,type:'CT'|'pollution',note}]
  }
  ...
  // Tours : identifiant + état « réparé » par dommage ; manquements régularisés.
  for (const ins of d.vehicleInspections) {
    if (!ins.counters) { /* rien */ }
    if (Array.isArray(ins.impacts)) {
      ins.impacts.forEach((im, i) => {
        if (!im.id) im.id = `imp_${ins.id}_${i}`;
        if (im.repaired === undefined) im.repaired = false;
      });
    }
    if (!ins.regularized || typeof ins.regularized !== 'object') ins.regularized = {};
  }
  // Messagerie interne.
  if (!Array.isArray(d.messages)) d.messages = [];
  for (const m of d.messages) { if (!Array.isArray(m.reads)) m.reads = []; }
  if (!d.settings.vehicleWarnAcks || typeof d.settings.vehicleWarnAcks !== 'object') d.settings.vehicleWarnAcks = {};
  // Stocks, dépenses véhicules, comptabilité.
  if (!Array.isArray(d.parts)) d.parts = [];
  if (!Array.isArray(d.vehicleExpenses)) d.vehicleExpenses = [];
  if (!d.finance || typeof d.finance !== 'object') d.finance = { entries: [] };
  if (!Array.isArray(d.finance.entries)) d.finance.entries = [];
  if (!Array.isArray(d.settings.partCategories) || !d.settings.partCategories.length) d.settings.partCategories = freshDefault().settings.partCategories;
  if (!Array.isArray(d.settings.partUnits) || !d.settings.partUnits.length) d.settings.partUnits = freshDefault().settings.partUnits;
  // Garantit la présence des catégories nécessaires aux services A/B.
  for (const c of ['Filtre à huile', 'Filtre habitacle', 'Filtre à air', 'Filtre à gasoil', 'Huile moteur 5W30']) {
    if (!d.settings.partCategories.includes(c)) d.settings.partCategories.unshift(c);
  }
  if (!Array.isArray(d.vehicleSchedule)) d.vehicleSchedule = [];
  if (!Array.isArray(d.contracts)) d.contracts = [];
  if (!Array.isArray(d.sanctions)) d.sanctions = [];
  if (!Array.isArray(d.workHours)) d.workHours = [];
  if (!Array.isArray(d.hsupSettlements)) d.hsupSettlements = [];
  if (typeof d.settings.hsupWeeklyBase !== 'number') d.settings.hsupWeeklyBase = 35;
  // Jour de clôture de la paie (0 = mois civil ; 22 = période du 23 au 22).
  if (typeof d.settings.hsupCutoffDay !== 'number') d.settings.hsupCutoffDay = 22;
  // Auto-apprentissage des associations « nom du fichier d'activité -> salarié ».
  if (!d.settings.driverImportLearning || typeof d.settings.driverImportLearning !== 'object') d.settings.driverImportLearning = {};
  if (!d.settings.salaryParams || typeof d.settings.salaryParams !== 'object') d.settings.salaryParams = {};
  if (!Array.isArray(d.bankTx)) d.bankTx = [];
  if (!Array.isArray(d.bankDocs)) d.bankDocs = [];
  if (!d.settings.disciplineAcks || typeof d.settings.disciplineAcks !== 'object') d.settings.disciplineAcks = {};
  if (!Array.isArray(d.settings.catRules) || !d.settings.catRules.length) d.settings.catRules = freshDefault().settings.catRules;
  if (!d.settings.catLearn || typeof d.settings.catLearn !== 'object') d.settings.catLearn = {};
  if (typeof d.settings.treasuryStartBalance !== 'number') d.settings.treasuryStartBalance = 0;
  if (!d.settings.tenderParams || typeof d.settings.tenderParams !== 'object') d.settings.tenderParams = freshDefault().settings.tenderParams;
  else { const def = freshDefault().settings.tenderParams; for (const k of Object.keys(def)) if (d.settings.tenderParams[k] === undefined) d.settings.tenderParams[k] = def[k]; }
  // Compteur « déjà pris » de base (saisi manuellement) par salarié.
  for (const u of d.users) {
    if (!u.takenBaseline || typeof u.takenBaseline !== 'object') u.takenBaseline = { congesN: 0, congesN1: 0, rcc: 0, heuresSupp: 0 };
  }
...
async function load() {
  if (USE_REDIS) {
    ...
    if (!raw) {
      ...
      data = typeof raw === 'string' ? deserialize(raw) : decryptPiiInData(raw);
    }
  } else {
    ...
    if (fs.existsSync(DATA_FILE)) {
      ...
      try {
        ...
        // Si la base est chiffrée mais illisible (clé manquante/incorrecte), on
        // NE réinitialise PAS (cela effacerait les données) : on stoppe net.
        if (isEncrypted(blob)) { console.error('Base chiffrée illisible :', e.message); throw e; }
  ...
  migrate(data);
  applyCpRollover(data); // bascule N -> N-1 au 1er juillet AVANT les acquisitions
  applyAccruals(data);
  applyRccReset(data);
  return data;
}
...
// Bascule annuelle des congés payés au 1er juillet (règle « en dur » pour toutes
// les années à venir) :
//   • les congés N-1 NON soldés avant le transfert sont perdus ;
//   • le compteur N (acquis sur la période échue) bascule vers le compteur N-1 ;
//   • le compteur N repart à zéro et se recharge ensuite +2,5 j/mois ;
//   • au planning, les congés posés sur le pool N basculent sur le pool N-1.
// Idempotent : ne s'exécute qu'une fois par an (mémorisé dans settings).
function applyCpRollover(d, now) {
  now = now || new Date();
  if (now.getMonth() < 6) return; // avant juillet (mois 0-5) : pas de bascule
  const Y = now.getFullYear();
  d.settings = d.settings || {};
  if ((d.settings.cpLastRolloverYear || 0) >= Y) return; // déjà basculé cette année
  for (const u of d.users) {
    if (!u.balances) continue;
    // N-1 d'avant : perdu. N -> N-1. Nouveau N à zéro.
    u.balances.congesN1 = Math.round((Number(u.balances.congesN) || 0) * 100) / 100;
    u.balances.congesN = 0;
    if (u.takenBaseline && typeof u.takenBaseline === 'object') {
      u.takenBaseline.congesN1 = Number(u.takenBaseline.congesN) || 0;
      u.takenBaseline.congesN = 0;
    }
  }
  // Plannings : les congés payés posés en N deviennent des congés N-1.
  for (const r of (d.requests || [])) {
    if (r.category === 'CP' && r.pool !== 'N1') r.pool = 'N1';
  }
  d.settings.cpLastRolloverYear = Y;
}
...
async function save() {
  if (!data) return;
  if (USE_REDIS) {
    // On stocke toujours une CHAÎNE sérialisée (chiffrée si DATA_ENCRYPTION_KEY,
    // sinon JSON), afin que les données personnelles soient chiffrées au niveau
    // des champs dans tous les cas. load() la désérialise (et déchiffre les PII).
    await redis.set(REDIS_KEY, serialize(data));
  } else {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
...
// Maintenance « congés » rejouable à chaud (tâche nocturne) : bascule annuelle
// au 1er juillet + acquisitions mensuelles + remise à zéro RCC. Idempotent.
function runLeaveMaintenance(d) {
  applyCpRollover(d);
  applyAccruals(d);
  applyRccReset(d);
}

module.exports = { load, save, getData, nextId, enableAccrual, runLeaveMaintenance, applyCpRollover, DATA_FILE, USE_REDIS };
