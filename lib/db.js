...
// Remet le compteur RCC à 0 au changement de trimestre civil (tous les 3 mois)
// si le salarié ne l'a pas posé. Repéré par "AAAA-Tn" (n = trimestre).
function quarterKey(date) {
  return `${date.getFullYear()}-T${Math.floor(date.getMonth() / 3) + 1}`;
}
function applyRccReset(d) {
  const cur = quarterKey(new Date());
  for (const u of d.users) {
    if (u.rccQuarter === undefined) { u.rccQuarter = cur; continue; }
    if (u.rccQuarter !== cur) {
      u.balances.rcc = 0;
      u.rccQuarter = cur;
    }
