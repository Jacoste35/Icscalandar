...
// Sérialise/désérialise en tenant compte du chiffrement éventuel.
function serialize(obj) { const json = JSON.stringify(obj, null, ENC_KEY ? 0 : 2); return ENC_KEY ? encryptData(json) : json; }
function deserialize(blob) {
  if (isEncrypted(blob)) {
    ...
    return JSON.parse(decryptData(blob));
  }
...
async function load() {
  if (USE_REDIS) {
    ...
    if (!raw) {
      ...
      data = typeof raw === 'string' ? deserialize(raw) : raw;
    }
  } else {
...
async function save() {
  if (!data) return;
  if (USE_REDIS) {
    // En clair : on laisse le SDK Upstash gérer le JSON. Avec chiffrement : on
    // stocke une chaîne chiffrée (load() la déchiffre).
    await redis.set(REDIS_KEY, ENC_KEY ? serialize(data) : data);
  } else {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
