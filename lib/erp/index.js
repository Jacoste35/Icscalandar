...
function ensureErp(data) {
  ...
  else { const def = require('./docoptions').DEFAULT_MOTIFS; for (const m of def) if (!data.settings.docMotifs.includes(m)) data.settings.docMotifs.push(m); } // ajoute les nouveaux motifs par défaut
  ...
  if (!data.settings.pajgps || typeof data.settings.pajgps !== 'object') {
    ...
    if (typeof g.chargesSalPct !== 'number') g.chargesSalPct = 22;  // charges salariales (net ≈ brut×(1-22%))
    if (typeof g.mealBreakHours !== 'number') g.mealBreakHours = 0.75; // pause déjeuner non payée
    if (typeof g.maxPaidHours !== 'number') g.maxPaidHours = 9;      // heures payées max / jour (borne réaliste)
    ...
    // Heure de prise de poste (détection de retard) : défaut global + par chauffeur + par groupe.
    ...
    if (!g.priseDePosteByGroup || typeof g.priseDePosteByGroup !== 'object') g.priseDePosteByGroup = {};
    // Attribution directe d'un traceur à un chauffeur inscrit (croisement des données).
    if (!g.deviceUserMap || typeof g.deviceUserMap !== 'object') g.deviceUserMap = {};
  }
  ...
  // Association carte AS 24 → chauffeur inscrit + projections de consommation.
  if (!data.settings.fuelDriverMap || typeof data.settings.fuelDriverMap !== 'object') data.settings.fuelDriverMap = {};
  if (!data.settings.fuelProjections || typeof data.settings.fuelProjections !== 'object') data.settings.fuelProjections = {};
  // Décisions sur les alertes carburant (fraude confirmée / faux positif).
  if (!data.settings.fuelAlertDecisions || typeof data.settings.fuelAlertDecisions !== 'object') data.settings.fuelAlertDecisions = {};
  // Archive de la consommation estimée (GPS) par véhicule et par jour.
