...
function render(tpl, vars) {
  let out = String(tpl || '');
  // Blocs conditionnels {{#if x}}...{{/if}} — traités du plus interne au plus
  // externe (la garde (?!\{\{#if) ne capture que les blocs sans #if imbriqué).
  // On boucle jusqu'à stabilité, sinon le {{/if}} d'un bloc externe « fuit ».
  const ifRe = /\{\{#if\s+([\w.]+)\}\}((?:(?!\{\{#if)[\s\S])*?)\{\{\/if\}\}/g;
  let prev;
  do { prev = out; out = out.replace(ifRe, (_, key, body) => (truthy(get(vars, key)) ? body : '')); } while (out !== prev);
  // Variables simples
...
const AVERTISSEMENT = `
<div class="lh"><div class="lh-co">{{company.legal}}</div><div class="lh-ad">{{company.address}}</div></div>
<p class="addr">{{salarie.fullName}}<br>{{salarie.address}}</p>
<p class="meta">Éterville, le {{date}}</p>
<p><strong>Objet : Avertissement disciplinaire – {{motif}}</strong></p>
<p class="meta">Réf. : Contrat de travail – CCN Transports Routiers et Activités Auxiliaires (IDCC 16) – Règlement Intérieur ICS<br>Envoi : Remise en main propre contre décharge (Art. L.1332-1 C. trav.)</p>
<p>{{salarie.civilite}},</p>
<p>Vous êtes employé au sein de la SASU Inter Colis Services. À ce titre, votre contrat de travail fixe votre prise de poste à l'heure convenue. Cette obligation est une condition essentielle de votre contrat, rappelée par les articles 11 et 12 du règlement intérieur d'ICS, ainsi que par les dispositions de la CCN Transports Routiers (IDCC 16).</p>
<p>{{faits}}</p>
<p>Ces faits constituent des manquements répétés à vos obligations contractuelles, expressément visés par les articles 11 et 12 du règlement intérieur d'ICS. Leur répétition est constitutive d'une désorganisation du service au sens de la jurisprudence de la Chambre sociale (Cass. soc. 4 janv. 2000, n° 97-44.026 ; Cass. soc. 19 mai 2016, n° 14-26.398). Conformément à l'article 21 du règlement intérieur, votre ancienneté a été prise en compte dans l'appréciation de la proportionnalité de la sanction ; elle ne saurait cependant exonérer des manquements répétés et dûment établis.</p>
<p>Sur le fondement de l'article L. 1331-1 du Code du travail, des articles 11, 12 et 20 (1°) du règlement intérieur d'ICS et des dispositions de la CCN Transports Routiers (IDCC 16), nous vous notifions par la présente un <strong>AVERTISSEMENT</strong>, première sanction de l'échelle disciplinaire, pour les manquements énumérés ci-dessus. Conformément à l'article 22 du règlement intérieur et à l'article L. 1332-2 du Code du travail, l'avertissement n'exige pas de convocation à entretien préalable. Conformément à l'article L. 1332-4 du Code du travail, cette sanction vous est notifiée dans le délai légal de deux mois suivant la connaissance des faits.</p>
<p>Nous vous avertissons solennellement que tout nouveau manquement de même nature constaté postérieurement à la présente notification constituera une récidive et pourra être sanctionné par une mise à pied disciplinaire sans solde, conformément à l'article 20 (3°) du règlement intérieur. L'article 12 du règlement intérieur prévoit expressément qu'en cas de récidive, la sanction peut aller jusqu'au licenciement pour faute grave.</p>{{#if miseAPied.proposed}}
<p>Compte tenu de votre dossier disciplinaire — qui comporte à ce jour <strong>{{miseAPied.warningCount}}</strong> avertissement(s) antérieur(s) — et de la gravité {{miseAPied.graviteLabel}} des faits visés, la Direction vous indique qu'en cas de persistance ou de réitération de ce comportement, elle envisagera à votre encontre une <strong>{{miseAPied.type}}</strong>{{#if miseAPied.jours}} d'une durée de <strong>{{miseAPied.jours}} jour(s)</strong>{{/if}}, mesure proportionnée à la désorganisation du service occasionnée, conformément aux articles L. 1331-1 et L. 1333-2 du Code du travail et à l'article 20 du règlement intérieur.</p>{{/if}}
<p>Cet avertissement sera versé à votre dossier disciplinaire. Conformément à l'article R. 1332-1 du Code du travail, vous disposez d'un délai de dix jours ouvrables à compter de la réception du présent courrier pour nous faire parvenir par écrit toute observation ou contestation.</p>
<p>Veuillez agréer, {{salarie.civilite}}, l'expression de nos salutations distinguées.</p>
<p class="sign">M. Quentin Routel<br>Directeur – {{company.legal}}</p>`;

// Bloc « en-tête société + destinataire » réutilisé par les courriers de mise
// à pied (reprend la mise en forme du modèle fourni). Les [crochets] non
// remplaçables automatiquement sont à compléter dans l'aperçu avant envoi.
const MAP_HEAD = `
<div class="lh"><div class="lh-co">{{company.legal}}</div><div class="lh-ad">{{company.address}}{{#if company.siret}} — SIRET {{company.siret}}{{/if}}</div></div>
<p class="meta">SASU INTER COLIS SERVICES (ICS) — représentée par Quentin ROUTEL, Président</p>
<p class="addr">{{salarie.civilite}} {{salarie.fullName}}<br>{{salarie.poste}}<br>{{salarie.address}}</p>
<p class="meta">Lettre recommandée avec accusé de réception (et remise en main propre contre décharge)<br>Éterville, le {{date}}</p>`;

const MAP_DECHARGE = `
<p class="sign">Pour la société INTER COLIS SERVICES (ICS),<br>Quentin ROUTEL, Président<br>Signature</p>
<p class="meta">Remise en main propre – décharge<br>Reçu en main propre le [date] — Signature du salarié : ____________________<br><em>(en cas de refus de signer, le mentionner en présence d'un témoin)</em></p>`;

// Mise à pied CONSERVATOIRE — mesure d'attente (art. L.1332-3), notifiée en même
// temps que la convocation à l'entretien préalable (concomitance obligatoire).
const MISE_A_PIED_CONSERVATOIRE = `${MAP_HEAD}
<p><strong>Objet : Mise à pied conservatoire à effet immédiat et convocation à un entretien préalable</strong></p>
<p>Madame, Monsieur,</p>
<p>Nous avons été informés, le [date de la connaissance des faits], de faits que nous considérons comme particulièrement graves, à savoir :</p>
<p>{{faits}}</p>{{#if historique}}
<p>{{historique}}</p>{{/if}}
<p>Ces faits, par leur nature et leur gravité, rendent impossible votre maintien dans l'entreprise pendant le temps nécessaire à l'examen de la situation.</p>
<p>En conséquence, nous vous notifions par la présente une <strong>mise à pied conservatoire à effet immédiat</strong>, à compter du [date] à [heure]. Pendant toute la durée de cette mesure, l'exécution de votre contrat de travail est suspendue : vous n'êtes pas autorisé(e) à vous présenter sur votre lieu de travail ni à exercer vos fonctions, et votre rémunération n'est pas maintenue pour cette période, sous réserve de la décision définitive qui sera prise à l'issue de la procédure.</p>
<p>Nous vous précisons que cette mise à pied constitue une mesure conservatoire et non une sanction : elle ne préjuge en rien de la décision susceptible d'être prise à votre encontre.</p>
<p>Par ailleurs, et conformément aux articles L.1332-1 et suivants du Code du travail, nous vous convoquons à un <strong>entretien préalable</strong> en vue d'une éventuelle sanction pouvant aller jusqu'au licenciement, qui se tiendra le : <strong>[jour de la semaine, date]</strong> à <strong>[heure]</strong>, à l'adresse suivante : {{company.address}}.</p>
<p>Cet entretien ne pourra avoir lieu moins de cinq jours ouvrables après la première présentation de la présente lettre (ou sa remise en main propre). Lors de cet entretien, vous pouvez vous faire assister par une personne de votre choix appartenant au personnel de l'entreprise ou, en l'absence d'institution représentative, par un conseiller du salarié inscrit sur la liste consultable en mairie et auprès de la DDETS / unité départementale de l'inspection du travail.</p>
<p>Nous vous prions d'agréer, Madame, Monsieur, l'expression de nos salutations distinguées.</p>
${MAP_DECHARGE}`;

// Mise à pied DISCIPLINAIRE — Lettre n°1 UNIQUEMENT : convocation à l'entretien
// préalable. La notification (Lettre n°2) est un document distinct, à générer
// après l'entretien (voir NOTIFICATION_MISE_A_PIED_DISCIPLINAIRE).
const MISE_A_PIED_DISCIPLINAIRE = `${MAP_HEAD}
<p><strong>Objet : Convocation à un entretien préalable à une éventuelle sanction disciplinaire (mise à pied)</strong></p>
<p>Madame, Monsieur,</p>
<p>Nous envisageons de prendre une sanction disciplinaire — une mise à pied — à votre encontre en raison des faits suivants :</p>
<p>{{faits}}</p>{{#if historique}}
<p>{{historique}}</p>{{/if}}
<p>Conformément aux articles L.1332-1 et suivants du Code du travail, nous vous convoquons à un entretien préalable qui se tiendra le : <strong>[jour de la semaine, date]</strong> à <strong>[heure]</strong>, à l'adresse suivante : {{company.address}}.</p>
<p>Au cours de cet entretien, nous vous exposerons les motifs de la sanction envisagée et recueillerons vos explications. Cet entretien ne pourra se tenir moins de cinq jours ouvrables après la première présentation de la présente lettre. Vous pouvez vous faire assister par une personne de votre choix appartenant au personnel de l'entreprise ou par un conseiller du salarié inscrit sur la liste consultable en mairie et auprès de la DDETS / inspection du travail.</p>
<p>Nous vous prions d'agréer, Madame, Monsieur, l'expression de nos salutations distinguées.</p>
${MAP_DECHARGE}`;

// Mise à pied DISCIPLINAIRE — Lettre n°2 : notification de la sanction.
// À générer/envoyer après l'entretien (entre 2 jours ouvrables et 1 mois).
const NOTIFICATION_MISE_A_PIED_DISCIPLINAIRE = `${MAP_HEAD}
<p><strong>Objet : Notification d'une mise à pied disciplinaire</strong></p>
<p>Madame, Monsieur,</p>
<p>À la suite de l'entretien préalable qui s'est tenu le [date de l'entretien], auquel vous [vous êtes présent(e) / ne vous êtes pas présenté(e)], nous vous notifions la sanction suivante. Les faits qui vous sont reprochés sont les suivants :</p>
<p>{{faits}}</p>
<p>Ces faits constituent un manquement à vos obligations professionnelles. En conséquence, nous avons décidé de vous notifier une <strong>mise à pied disciplinaire d'une durée de {{miseAPied.jours}} jour(s) ouvrable(s)</strong>, dans la limite du plafond fixé par le règlement intérieur.</p>
<p>Cette mise à pied prendra effet du <strong>[date de début]</strong> au <strong>[date de fin]</strong> inclus. Pendant cette période, l'exécution de votre contrat de travail est suspendue : vous ne devez pas vous présenter sur votre lieu de travail et votre rémunération ne vous sera pas versée pour la durée correspondante. Vous reprendrez votre poste le <strong>[date de reprise]</strong> à <strong>[heure]</strong>.</p>
<p>Nous vous demandons à l'avenir de respecter scrupuleusement vos obligations. Nous attirons votre attention sur le fait que tout nouveau manquement de votre part serait susceptible d'entraîner une sanction plus lourde, pouvant aller jusqu'au licenciement.</p>
<p>Nous vous prions d'agréer, Madame, Monsieur, l'expression de nos salutations distinguées.</p>
${MAP_DECHARGE}`;
...
const DEFAULT_TEMPLATES = {
  ...
  mise_a_pied_disciplinaire: { label: 'Mise à pied disciplinaire (convocation à l\'entretien)', body: MISE_A_PIED_DISCIPLINAIRE, category: 'Disciplinaire' },
  notification_mise_a_pied_disciplinaire: { label: 'Notification de la mise à pied disciplinaire', body: NOTIFICATION_MISE_A_PIED_DISCIPLINAIRE, category: 'Disciplinaire' },
  mise_a_pied_conservatoire: { label: 'Mise à pied conservatoire', body: MISE_A_PIED_CONSERVATOIRE, category: 'Disciplinaire' },
...
// Version des modèles par défaut : à incrémenter pour propager une mise à jour
// des modèles NON édités manuellement sur les installations existantes.
const TEMPLATES_VERSION = 6;

module.exports = { render, DEFAULT_TEMPLATES, TEMPLATES_VERSION };
