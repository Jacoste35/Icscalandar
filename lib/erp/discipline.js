...
const MOTIF_SEVERITY = {
  ...
  "Véhicule non propre à l'intérieur": { gravite: 1, seuil: 3 },
  "Véhicule non propre à l'extérieur": { gravite: 1, seuil: 3 },
  "Véhicule non propre à l'intérieur et à l'extérieur": { gravite: 2, seuil: 2 },
  'Non-entretien mécanique du véhicule malgré voyants / bruits (mise en danger)': { gravite: 3, seuil: 1 },
...
// Familles de motifs « de même nature » : les avertissements d'une même famille
// s'additionnent vers la mise à pied. La jurisprudence admet de retenir des faits
// de MÊME NATURE non prescrits (3 ans) pour apprécier la récidive — on reste
// volontairement conservateur : seuls des motifs incontestablement identiques par
// leur nature sont regroupés (ici, les manquements de propreté du véhicule).
const MOTIF_FAMILIES = [
  { key: 'proprete', label: 'Défaut de propreté du véhicule', match: /propre/i },
];
function motifFamily(motif) {
  const fam = MOTIF_FAMILIES.find((f) => f.match.test(motif || ''));
  return fam ? fam.key : (motif || '(non précisé)');
}
function familyLabel(motif) {
  const fam = MOTIF_FAMILIES.find((f) => f.match.test(motif || ''));
  return fam ? fam.label : (motif || '(non précisé)');
}
 ...
 */
function computeMiseAPied({ warningCount, motif, gravite, seuil }) {
  const base = severityOf(motif);
  // Permet de passer une gravité/seuil agrégés (famille de motifs) qui priment
  // sur ceux déduits du libellé seul.
  const sev = { gravite: gravite != null ? gravite : base.gravite, seuil: seuil != null ? seuil : base.seuil };
...
module.exports = { MOTIF_SEVERITY, GRAVITE_LABEL, MAX_JOURS, severityOf, computeMiseAPied, motifFamily, familyLabel };
