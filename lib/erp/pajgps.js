...
function statsFor(st, id, now) {
  ...
  let kmDay = 0, km30 = 0, km90 = 0, fuel30 = 0, ideal30 = 0, urban30 = 0;
  for (const ymd of Object.keys(ds)) {
    const e = ds[ymd];
    if (ymd === todayY) kmDay += e.km;
  ...
  return {
    kmDay: round1(kmDay), kmMonth: round1(km30), kmQuarter: round1(km90),
...
function costFor(data, st, id, d, veh, cfg, atDepot, now) {
  ...
  // Amplitude d'utilisation : du 1er départ à l'immobilisation (dépôt) ou 18h.
  const CAP = 18 * 60;
  const startMin = d.dayFirstMoveAt ? parisMinOfDay(d.dayFirstMoveAt) : null;
  const nowMin = parisMinOfDay(now.toISOString());
  let endMin;
  if (atDepot && !d.moving && d.finalStopAt) endMin = Math.min(parisMinOfDay(d.finalStopAt), CAP);
  else endMin = Math.min(nowMin, CAP);
  const hours = startMin != null ? Math.max(0, (endMin - startMin) / 60) : 0;

  // Pôle salarié : chauffeur affecté au véhicule × coût horaire chargé.
  ...
  const salarie = hours * hourlyEmployer;
  ...
  return {
    ...
    hours: round1(hours), km: round1(kmToday), liters: round1(litersToday),
...
function liveList(data) {
  ...
  for (const id of Object.keys(st.devices)) {
    ...
    const priseRef = (driverId && cfg.priseDePosteByUser && cfg.priseDePosteByUser[driverId]) || cfg.priseDePoste || '';
    let late = null;
    ...
    out.push({
      ...
      activeSince: d.dayFirstMoveAt || null, late,
