...
// entrée), temps total passé sur place, ET le détail de chaque passage dans le
// périmètre (heure d'entrée / sortie / durée) avec le nombre de passages du jour.
function depotSummary(trail, depot) {
  ...
  // Départ = sortie de la 1re présence au dépôt (null si le véhicule y est encore).
  const depart = vs[0].open ? null : vs[0].exit;
  // Retour = entrée de la DERNIÈRE présence, seulement s'il y a eu un retour (≥ 2 passages).
  const retour = vs.length >= 2 ? vs[vs.length - 1].enter : null;
  // Le véhicule est-il actuellement au dépôt ? (dernier passage encore ouvert)
  const atDepotNow = !!vs[vs.length - 1].open;
  const dwellMin = Math.round(vs.reduce((s, v) => s + v.minutes, 0) * 10) / 10;
  // Passages = entrées dans le périmètre (heure d'entrée, sortie, durée).
  const visits = vs.map((v) => ({ enter: v.enter, exit: v.open ? null : v.exit, minutes: v.minutes, open: !!v.open }));
  return { label: depot.label, depart, retour, atDepotNow, dwellMin, visits, count: visits.length, firstEnter: vs[0].enter };
}
