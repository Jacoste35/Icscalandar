...
function updateDevice(st, dev, p, now, fixedLimit, win, extra) {
  ...
  if (moving && speed > MOVING_KMH) {
    ...
    const cActual = urban ? URBAN_CONSO : consoLper100(speed);
    const idealSpeed = appLimit ? Math.min(speed, appLimit) : speed;
    const cIdeal = urban ? URBAN_CONSO : consoLper100(idealSpeed);
...
// Constantes de réalisme de consommation (VL ≤ 3,5 t).
const URBAN_CONSO = 10.5;     // L/100 km en ville (arrêts/redémarrages)
