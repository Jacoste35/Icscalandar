...
async function http(method, path, { token, query, body } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      const msg = (json && (json.error || json.message || json.msg)) || ('HTTP ' + res.status);
      const err = new Error('PAJ : ' + msg); err.status = res.status; throw err;
    }
    return json;
  } catch (e) {
...
module.exports = {
  encrypt, decrypt, isEnc, ensureState, liveList,
