...
function computeAlerts(data, now = new Date(), opts = {}) {
  ...
  // 6) Tournées déficitaires par client sur 21 jours glissants.
  try {
    const tours = require('./tours');
    for (const row of tours.marginByClientWindow(data, now, 21)) {
      if (row.client !== 'Divers' && row.marge < 0) out.push(alert('urgent', 'Finance', `Tournées déficitaires — ${row.client}`,
        `Marge cumulée ${money(row.marge)} sur 21 jours pour ${row.client}.`, { ref: { type: 'client', id: row.client } }));
    }
  } catch (e) { /* ignore */ }

  // 7) Taux d'échec chauffeur > 8 % sur 14 jours.
  try {
    const tours = require('./tours');
    for (const d of tours.failRateByDriverWindow(data, now, 14)) {
      if (d.rate > 0.08) out.push(alert('info', 'RH', `Taux d'échec élevé — ${d.userName || d.userId}`,
        `${(d.rate * 100).toFixed(1)} % d'échec de livraison sur 14 jours.`, { ref: { type: 'user', id: d.userId } }));
    }
  } catch (e) { /* ignore */ }

  // 8) Trésorerie prévisionnelle : semaine sous 0 dans les 8 prochaines semaines.
  try {
    const fc = require('./cashflow').forecast(data, 8, now);
    const neg = fc.buckets.find((b) => b.negative);
    if (neg) out.push(alert('critique', 'Finance', 'Trésorerie prévisionnelle négative',
      `Solde projeté ${money(neg.balance)} en semaine ${neg.week} (à partir du ${neg.from}).`, { ref: { type: 'cashflow', id: neg.from } }));
  } catch (e) { /* ignore */ }

  // 9) Stock : pièces sous le seuil mini -> réapprovisionnement.
  try {
    const low = (data.parts || []).filter((p) => (Number(p.seuilMini) || 0) > 0 && (Number(p.qty) || 0) <= Number(p.seuilMini));
    for (const p of low) out.push(alert('info', 'Stock', `Réappro — ${p.name}`,
      `${p.name} : ${p.qty || 0} ${p.unit || ''} en stock (seuil ${p.seuilMini}).`, { ref: { type: 'part', id: p.id }, action: 'restock' }));
  } catch (e) { /* ignore */ }
